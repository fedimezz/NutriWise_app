...
<head>
    ...
    <title>Ajouter une Consultation - NutriWise Admin</title>
    ...
    <div class="dashboard-container">
        ...
        <main class="main-content">
            ...
            <div class="form-container" style="margin-top:2rem;">
                <form action="index.php?page=admin_add_consultation" method="POST">
                    <div class="form-group">
                        <label for="user_id">Utilisateur</label>
                        <select name="user_id" id="user_id" required>
                            <option value="">Selectionner un utilisateur</option>
                            <?php foreach ($users as $user): ?>
                                <option value="<?= $user['id'] ?>"><?= htmlspecialchars($user['prenom'] . ' ' . $user['nom']) ?> (<?= htmlspecialchars($user['email']) ?>)</option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="date_consultation" id="date_consultation" value="<?= date('Y-m-d') ?>" required>
                    </div>

                    <div class="form-group">
                        <label for="poids_cible">Poids cible (kg)</label>
                        <input type="number" step="0.1" name="poids_cible" id="poids_cible">
                    </div>

                    <div class="form-group">
                        <label for="remarque">Remarque</label>
                        <textarea name="remarque" id="remarque" required></textarea>
                    </div>

                    <div class="form-group">
                        <label for="conseil">Conseil</label>
                        <textarea name="conseil" id="conseil" required></textarea>
                    </div>
