...
<head>
    ...
    <style>
        ...
        .hero-copy { padding: 2.5rem; display: flex; flex-direction: column; justify-content: center; gap: 1.2rem; }
        .category-pill {
            display: inline-flex;
            align-items: center;
            gap: 0.5rem;
        ...
        .detail-title { font-size: 2.8rem; margin-bottom: 0.25rem; color: #1a3a1a; font-weight: 700; }
        .subheadline { color: #5a7a55; line-height: 1.6; font-size: 1rem; }
        .detail-meta { display: flex; flex-wrap: wrap; gap: 0.75rem; margin-top: 0.5rem; }
        .pill {
            display: inline-flex;
            align-items: center;
            gap: 0.4rem;
        ...
        .nutrition-card:hover { transform: translateY(-3px); box-shadow: 0 8px 20px rgba(0,0,0,0.05); }
        .nutrition-value { font-size: 2.2rem; font-weight: 800; color: #2e7d32; }
        .nutrition-label { margin-top: 0.6rem; color: #6b8a66; font-weight: 500; font-size: 0.85rem; text-transform: uppercase; }
        .split-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; margin-bottom: 2rem; }
        .info-card { padding: 1.8rem; border-radius: 24px; background: #ffffff; border: 1px solid rgba(76,175,80,0.12); }
        .info-card h3 { margin-bottom: 1rem; color: #2d5930; font-size: 1.3rem; }
        .progress-container { background: #e8f0e5; border-radius: 20px; height: 10px; margin: 1rem 0; overflow: hidden; }
        .progress-fill { background: linear-gradient(90deg, #4caf50, #2e7d32); height: 100%; border-radius: 20px; transition: width 0.6s ease; }
        .eco-level { display: flex; justify-content: space-between; margin-top: 0.5rem; font-size: 0.75rem; color: #7c9a76; }
        .badge-pill { padding: 0.6rem 1.1rem; border-radius: 999px; background: #eef5ec; color: #2e7d32; font-weight: 700; font-size: 0.8rem; display: inline-flex; align-items: center; gap: 0.5rem; }
        .related-section { margin-top: 2rem; }
        .related-section h2 { font-size: 1.6rem; margin-bottom: 1.2rem; color: #1a3a1a; }
        .related-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(230px, 1fr)); gap: 1.2rem; }
        .related-card {
            background: white;
            border-radius: 20px;
            overflow: hidden;
        ...
        .related-card:hover { transform: translateY(-5px); box-shadow: 0 12px 28px rgba(0,0,0,0.1); }
        .related-thumb { width: 100%; height: 140px; object-fit: cover; background: #eef5ec; }
        .related-content { padding: 1rem; }
        .related-content h4 { font-size: 1rem; margin-bottom: 0.3rem; color: #1e3a1e; }
        .empty-state { text-align: center; padding: 4rem; background: white; border-radius: 32px; }
        @keyframes fadeIn { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }
        .detail-card { animation: fadeIn 0.4s ease; }
        ...
        <div class="detail-container">
                ...
                <div class="detail-card">
                    <div class="detail-hero">
                        <div class="hero-image">
                            <img src="<?= $imageUrl ?>" alt="<?= htmlspecialchars($aliment['nom']) ?>" loading="lazy" onerror="this.src='https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=600&h=400&fit=crop'">
                        </div>
                        <div class="hero-copy">
                            ...
                            <p class="subheadline">Découvrez les valeurs nutritionnelles et l'impact environnemental de cet aliment.</p>
                    ...
                    <div class="detail-body">
                        ...
                        <div class="split-grid">
                            <div class="info-card">
                                ...
                                <?php if($ecoScore >= 7): ?>
                                    <p style="margin-top: 1rem; color: #2e7d32;"><i class="fas fa-check-circle"></i> Excellent choix pour l'environnement !</p>
                                <?php elseif($ecoScore >= 4): ?>
                                    <p style="margin-top: 1rem; color: #f39c12;"><i class="fas fa-exclamation-triangle"></i> Impact modéré</p>
                                <?php else: ?>
                                    <p style="margin-top: 1rem; color: #e74c3c;"><i class="fas fa-exclamation-circle"></i> Impact élevé, privilégiez des alternatives durables.</p>
                                <?php endif; ?>
                            </div>
                            <div class="info-card">
                                ...
                                <p style="margin-top: 1rem; line-height: 1.5; color: #4a6741;">
                                    <?php if(($aliment['proteines'] ?? 0) > 15): ?>
                                        Parfait pour la construction musculaire et la satiété. Idéal après le sport !
                                    <?php elseif(($aliment['calories'] ?? 0) < 80): ?>
                                        Idéal pour une collation légère ou un accompagnement.
                                    <?php else: ?>
                                        À intégrer dans une alimentation variée et équilibrée.
                                    <?php endif; ?>
                                </p>
                            </div>
                        </div>
                                        ...
                                        <div class="related-card" onclick="window.location='index.php?page=aliment_details&id=<?= (int)$related['id'] ?>';">
                                            ...
                                            <div class="related-content">
                                                ...
                                                <div style="display:flex; gap:0.8rem; font-size:0.8rem;">
                                                    <span><i class="fas fa-fire"></i> <?= number_format($related['calories'] ?? 0, 0) ?> kcal</span>
                                                    <span><i class="fas fa-leaf"></i> <?= number_format($related['eco_score'] ?? 0, 1) ?>/10</span>
                                                </div>
                                            </div>
                                        </div>
                                    <?php endforeach; ?>
            ...
            <?php else: ?>
                <div class="empty-state">
                    ...
                    <p>L'aliment que vous recherchez n'existe pas.</p>
